...
int m_compare(matrix *m1, matrix *m2) {
    if(m1->row != m2->row || m1->col != m2->col){
        printf("Matrizes não tem as mesmas dimensões!\n");
        printf("m1[%d][%d] && m2[%d][%d]\n", m1->row, m1->col, m2->row, m2->col);
        return 1;
    }
    for (int i=0; i < m1->row * m1->col; i++) {
        if (m1->datarow[i] != m2->datarow[i]) {
            printf("O elemento [%d,%d] é diferente nas matrizes analisadas!\n", (int) floor(i/m1->row), (int) (i%m1->row));
            printf("%d %d", m1->datarow[i], m2->datarow[i]);
            return 1;
        }
    }
